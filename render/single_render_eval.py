...
def save_images(object_file: str) -> None:
    ...
    # To render 40 different views of 3D objects
    img_ids = [[f"{view_num}", f"{view_num}_0_10", f"{view_num}_1_10",
                f"{view_num}_2_10", f"{view_num}_3_10"] for view_num in range(8)]
    img_ids = sum(img_ids, [])
    offset = np.radians(10)
    polar_angles = np.radians([60, 50, 50, 70, 70] * 4 + [90, 80, 80, 100, 100] * 4)
    azimuths = np.radians([-60, 30, 120, -150, -30, 60, 150, -120])
    azimuths = [[x, x-offset, x+offset, x-offset, x+offset] for x in azimuths]
    azimuths = sum(azimuths, [])
    ...
    for index, image in enumerate(data["colors"]):
        ...
        # depth_min, depth_max = np.min(image), np.max(image)
        # image = (image - depth_min) / (depth_max - depth_min) * 255
        # print(np.max(image[image!=1.e+10]), np.min(image))
        # image=image*127.5
        render_path = os.path.join(args.output_dir, f"view_{img_ids[index]}_gt.png")
        plt.imsave(render_path, image)
        if len(img_ids[index]) == 1:
            continue
        mask = image[:,:,3] == 0
        image[mask] = [255,255,255,255]
        render_path = os.path.join(args.output_dir, f"view_{img_ids[index]}.png")
        plt.imsave(render_path, image)
    ...
    for index, image in enumerate(data["depth"]):
        ...
        # image = (image - depth_min) / (depth_max - depth_min) * 255
        # print(np.max(image[image!=1.e+10]), np.min(image))
        image[image==1.e+10]=0
        render_path = os.path.join(args.output_dir, f"view_{img_ids[index]}_depth_mm.png")
        plt.imsave(render_path, image, cmap='gray', vmax=20)
        # save_array_as_image(image, "depth", render_path) # Note that you may need to change the cmap of imsave() in visHdf5Files.py to get correct depth image. 
        if len(img_ids[index]) == 1:
            # Increase the lights
            image = image * 5
            render_path = os.path.join(args.output_dir, f"view_{img_ids[index]}_depth.png")
            plt.imsave(render_path, image, cmap='gray', vmax=20)
